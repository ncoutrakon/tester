...
# added trade activity log to see when and what trades were made
class NotSoNaivePortfolio(Portfolio):
    """
    The NaivePortfolio object is designed to send orders to
    a brokerage object with a constant quantity size blindly,
    i.e. without any risk management or position sizing. It is
    ...
    def update_trade_activity_from_fill (self, fill):
        """
        Takes a FillEvent object and updates the trade log matrix
    ...
    def update_fill(self, event):
        ...
        """
        if event.type == 'FILL':
            self.update_trade_activity_from_fill(event)
            self.update_positions_from_fill(event)
            self.update_holdings_from_fill(event)
